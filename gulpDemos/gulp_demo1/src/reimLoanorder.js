function reimLoanorder(module,args,invokeRecord) {
	var isProject = true;
	var _data;
	// alert($departmentNo);
	...
	var mainListFields = [
		...
		{name:"applicationDate",title:"借款日期",width:120,dateFormatter:"toJapanShortDatetime"},
		{name:"applicationType",title:"单据性质",width:120,
			 canFilter:false},
		{name:"applicantName",title:"借款人员",width:100},
		{name:"departmentName",title:"借款部门",width:150},
		...
		{name:"content",title:"借款内容",width:"*",showIf:"false"},
		{name:"amountTotal",title:"借款总金额",type:"float",width:100, formatCellValue: "isc.Format.toUSString(value,2)",defaultValue:0,summaryFunction:"sum"}
	];
	
	var hoverFields = [
		{name:"headSubID",showIf:"false"},
		{name:"applicationClassification",title:"借款类别",width:120},
		{name:"applicationContent",title:"借款内容",width:"*"},
		{name:"applicationAmount",title:"借款金额",type:"float", formatCellValue: "isc.Format.toUSString(value,2)",width:120,defaultValue:0,summaryFunction:"sum"},
	...
	var innerFormFields = [
		...
		{name:"applicantName",title:"借款人员",width:"*",canEdit:false},
		...
		{name:"applicationFormNo",title:"借款单号",width:"*",canEdit:false,showIf:"true"},
		...
		{name:"applicationDate",title:"借款日期",defaultValue:new Date(),canEdit:false,width:"*",type:"datetime",showIf:"false"},//dateFormatter:"toJapanShortDatetime",
		...
		{ name:"payMoney",title:"已借款金额", type:"float", formatCellValue: "isc.Format.toUSString(value,2)",width:100,showIf:"false"},
		...
		{name:"relateApplicationNo",title:"<font style='cursor:pointer'><img src='images/Search.gif'></img>预算单号</font>",width:"*",required:true,
			...
			pickListFields: [
				...
				*/
				...
				// { name:"surplusMoney",title:"剩余金额", type:"float", formatCellValue: "isc.Format.toUSString(value,2)",width:100,showIf:"false"},
				{ name:"loanbalance",title:"借款余额",type:"float", formatCellValue: "isc.Format.toUSString(value,2)",width:100},
				{ name:"relateApplicationNo",showIf:"false"},
				{ name:"isSeparate",title:"多次借款",width:80,type:"boolean",showIf:"false"}
			],
			getPickListFilterCriteria: function(){
				...
				form.setValue("loanbalance",record.loanbalance);
				// 新增字段给打印用
			...
			titleClick :function(form, item){
				...
				var applicationListFields = [
					...
					{name:"reimbursementAmount",title:"借款金额",type:"float",align:"right", formatCellValue: "isc.Format.toUSString(value, 2)",summaryFunction:"sum",width:120,showIf:"false"},
				...
				var itemWin = isc.M3Windows.create({
					title:"借款记录:",VLWidth:900,VLHeight:370,
		...
		{name:"relateApAmountT",title:"预算总额",width:"*",canEdit:false,
			...
			{name:"loanbalance",title:"借款余额",width:"*",canEdit:false,formatCellValue: "isc.Format.toUSString(value,2)"},
		// {name:"surplus",title:"<font style='cursor:pointer'><img src='images/Search.gif'></img>剩余额度</font>",width:"*",canEdit:false, 
		{name:"surplus",title:"剩余额度",showIf:"false",width:"*",canEdit:false, 
			type:"float",align:"right", //endRow:true,
			// disabled :true,
			formatValue:function(value, record, form, item){
				return isc.Format.toUSString(value,2);
		...
		{name:"subject",title:"借款主题",width:"*",colSpan:3,canEdit:true,required:true,type:"text"},
		...
		{name:"isSeparate",title:"<b><font color=red>多次借款</font>", type:"checkbox",showIf:"true"},
		...
		{name:"reimbursementAmount",title:"本次借款",width:"*",keyPressFilter: "[0-9.]",canEdit:false,showIf:"false",//required:true,
		...
		{name:"loanTotalAmount",title:"借款总额",width:"*",canEdit:false,showIf:"false", 
			type:"float",align:"right", //endRow:true,
			formatValue:function(value, record, form, item){
				return isc.Format.toUSString(value,2);
		}},
		{name:"returnedAmount",title:"退回金额",width:"*",canEdit:false,showIf:"false", 
			type:"float",align:"right",
			formatValue:function(value, record, form, item){
				return isc.Format.toUSString(value,2);
			}},
		{name:"additionalAmount",title:"补付金额",width:"*",canEdit:false,showIf:"false",
			type:"float",align:"right",
			formatValue:function(value, record, form, item){
				return isc.Format.toUSString(value,2);
			}},
	...
	var innerListDetail = isc.M3ListGrid.create({
		...
		fields:[
			{name:"subID",showIf:"false"},
			{name:"applicationClassification",title:"借款类别",width:120,editorType:"TextItemAddIcon",
			// editorProperties:{
				// align:"right",
					...
					keyPress: function(item, form, keyName){
						if (keyName == 'Enter'){
							...
							var listProject = isc.M3ListGrid.create({
								...
								fields: [
									{ name:"no",title:"借款编号",width:60},
									{ name:"name",title:"借款名称", width:150},
						...
						if (keyName == 'Enter'){
							...
							var listProject = isc.M3ListGrid.create({
								...
								fields: [
									...
									{name:"projectName",title:"项目名称",width:150},
									{name:"projectTotalAmount",title:"项目金额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"},
									{name:"usedAmount",title:"已使用金额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"},
									{name:"remainingAmount", title: "剩余金额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"},
									...
									{name:"KMYSJE",title:"科目预算金额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"},
									{name:"KMSYJE",title:"科目使用金额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"},
									{name:"KMYE", title: "科目余额",width:100,type:'float', formatCellValue: "isc.Format.toUSString(value, 2)"}
								]
							});
			...
			{name:"applicationAmount",title:"借款金额",type:"float", formatCellValue: "isc.Format.toUSString(value,2)",width:150,
				editorProperties: {
					keyPressFilter: "[0-9.]",
		...
		getCellHoverComponent : function (record, rowNum, colNum) {
			...
			if(me.getFieldName (colNum)=="ImplementWay"){
				...
				this.rowHoverComponent = isc.DetailViewer.create({
					fields: [{name:"correspondingStandard",title:"借款内容填写导航"}],
					// height:"100%",
					autoFitData :"vertical",
					initWidget : function () {
						this.Super("initWidget", arguments);
				...
				if(record){
					...
					var auditWin = isc.M3Windows.create({
						...
						saveAction:function(){
							...
							if(!layout.innerForm.getValue("relateApplicationNo")){
								values.itemType = "无预算借款单";
							}else if(amountTotalInput<=surplus){
								values.itemType = "有预算借款单";
							}else if(amountTotalInput>surplus){
								values.itemType = "超预算借款单";
							}
	...
	var innerSaveB = function(type){
		...
		var auditWin = isc.M3Windows.create({
			...
			saveAction:function(){
				...
				if(!layout.innerForm.getValue("relateApplicationNo")){
					criteria.itemType = "无预算借款单";
					layout.innerForm.values.itemType = "无预算借款单";
				}else if(amountTotalInput<=surplus){
					criteria.itemType = "有预算借款单";
					layout.innerForm.values.itemType = "有预算借款单";
				}else if(amountTotalInput>surplus){
					criteria.itemType = "超预算借款单";
					layout.innerForm.values.itemType = "超预算借款单";
				}
											...
											isc.DataSource.get("T_applicationFormHeadDS").updateData({applicationFormHeadID:mid,subID:mid},function(dsResponse,data){
												isc.DataSource.get("T_applicationFormHeadDS").fetchData(criteria,function(dsResponse,data){	
													...
													isc.DataSource.get("T_paymentApplicationContentDS").fetchData({subID:mid,xml:listxml},function(dsResponse,data){
														...
														// var loanValues = {};
														// loanValues.subID = data[0].headSubID;
														// loanValues.GZNY = $GZNY.substring(0,4);
														// loanValues.userName = $userName;
														// isc.DataSource.get("T_loanAmountDS").fetchData(loanValues,function(dsResponse,data){	
															...
															if(data[0].auditorName=="无需审核"){
																isc.DataSource.get("T_applicationFormHeadDS").fetchData({subID : mid}, function (dsResponse, data) {
																		...
																		isc.confirm("此份借款单共有<font color=blue>" + pages + "页</font>需要打印，您是否需要打印？如打印，请您检查不要遗漏。", function (value) {
																			if (value) {
															...
															auditWin.window.hide();
														//},{operationId:"SCW_loanAmount_save"});
		...
		if(!layout.innerForm.getValue("relateApplicationNo")){
			auditWin.window.setTitle("审核人员列表 此次借款单据类别为:<font color=blue>无预算借款单</font>");
		}else if(amountTotalInput<=surplus){
			auditWin.window.setTitle("审核人员列表 此次借款单据类别为:<font color=blue>有预算借款单</font>");
		}else if(amountTotalInput>surplus){
			auditWin.window.setTitle("审核人员列表 此次借款单据类别为:<font color=blue>超预算借款单</font>");
		}
	...
	var innerSaveActionA = function(type){
		...
		// if(amountTotal>relateApAmountT){
			// isc.say("借款总额超过申请单总额,请检查!");return;
		// }
		// if(layout.innerForm.getValue("subject").length>40){
			// isc.say("主题内容字符太多,请控制40字以内!");return;
		// }
